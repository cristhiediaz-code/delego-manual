...
## 14.1 Integración con Zoho (y otros sistemas) vía API / Webhook
...
> 💡 **Tip — patrón típico con Zoho:** Zoho **crea tareas** en Delego llamando a la API (con la x-api-key), y Delego **devuelve los resultados** (tarea completada, evidencia) a Zoho mediante el Webhook o las notificaciones por Webhook (§10.1). Así el flujo es bidireccional.
...
- Se inserta en los correos de notificación al cliente desde los **Campos Dinámicos** del editor de notificaciones (§10.6).
...
- En el reporte (§11.2) puedes incluir la columna **Coordenadas de Ejecución** (latitud/longitud del punto donde el agente marcó la tarea), junto con **Fecha/Hora de Ejecución**.
